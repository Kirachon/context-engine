#!/usr/bin/env node
/**
 * Deterministic CI guard that blocks new Augment/Auggie branding in active
 * runtime/docs surfaces while allowing file-scoped compatibility shims.
 *
 * Usage:
 *   node --import tsx scripts/ci/check-brand-detachment.ts [--allowlist <path>] [--roots <csv>] [--strict]
 *
 * Exit codes:
...
const BLOCKED_PATTERNS: Array<{ label: string; regex: RegExp }> = [
  ...
  { label: '@augmentcode/auggie', regex: /@augmentcode\/auggie/i },
  { label: 'augment_legacy', regex: /\baugment_legacy\b/i },
  { label: 'augmentcode', regex: /\baugmentcode\b/i },
  { label: 'AUGMENT_API_TOKEN', regex: /\bAUGMENT_API_TOKEN\b/i },
  { label: 'AUGMENT_API_URL', regex: /\bAUGMENT_API_URL\b/i },
  { label: 'auggie', regex: /auggie/i },
  { label: 'Augment', regex: /\bAugment\b/ },
  { label: '.augment-', regex: /\.augment-/i },
  { label: '.augmentignore', regex: /\.augmentignore/i },
];

function printUsage(code: number): never {
  // eslint-disable-next-line no-console
  console.log(
    'Usage: node --import tsx scripts/ci/check-brand-detachment.ts [--allowlist <path>] [--roots <csv>] [--strict]'
  );
  process.exit(code);
}
...
function readAllowlist(allowlistPath: string): string[] {
  ...
  for (const rawLine of lines) {
    ...
    items.push(normalizeRelative(line));
  }
  return items;
}

function isAllowlisted(relativePath: string, allowlist: string[]): boolean {
  for (const entry of allowlist) {
    ...
    if (minimatch(relativePath, entry, { dot: true, nocase: false })) {
      return true;
    }
  }
  return false;
...
function findViolations(files: string[], allowlist: string[]): Violation[] {
...
function main(): void {
  ...
  try {
    ...
    if (violations.length > 0) {
      // eslint-disable-next-line no-console
      console.error('Brand detachment check failed.');
      ...
      // eslint-disable-next-line no-console
      console.error(
        `Summary: scanned_files=${files.length} allowlisted_files=${allowlist.length} violations=${violations.length}`
      );
      process.exit(1);
    }

    // eslint-disable-next-line no-console
    console.log('Brand detachment check passed.');
    // eslint-disable-next-line no-console
    console.log(`Summary: scanned_files=${files.length} allowlisted_files=${allowlist.length} violations=0`);
    process.exit(0);
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error(error instanceof Error ? error.message : String(error));
