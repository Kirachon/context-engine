...
function run(): number {
  try {
    ...
    const fixtureResolved = path.resolve(args.fixturePackPath);
    const fixtureRawText = fs.readFileSync(fixtureResolved, 'utf8');
    const fixtureHash = createHash('sha256').update(fixtureRawText).digest('hex');
    const commitSha =
      process.env.GITHUB_SHA?.trim() ||
      process.env.CI_COMMIT_SHA?.trim() ||
      process.env.BUILD_SOURCEVERSION?.trim() ||
      (() => {
        try {
    ...
    const holdout = (fixtureObj as Record<string, unknown>).holdout as Record<string, unknown> | undefined;
    const defaultDatasetId = typeof holdout?.default_dataset_id === 'string' ? holdout.default_dataset_id : 'unknown';
    const holdoutCheckPath = path.resolve(args.holdoutArtifactPath);
    let artifactDatasetId = '';
    let artifactDatasetHash = '';
    if (fs.existsSync(holdoutCheckPath)) {
      try {
        const holdoutParsed = JSON.parse(fs.readFileSync(holdoutCheckPath, 'utf8')) as Record<string, unknown>;
        const summary = holdoutParsed.summary as Record<string, unknown> | undefined;
        if (typeof summary?.dataset_id === 'string' && summary.dataset_id.length > 0) {
          artifactDatasetId = summary.dataset_id;
        }
        if (typeof summary?.dataset_hash === 'string' && summary.dataset_hash.length > 0) {
          artifactDatasetHash = summary.dataset_hash;
        }
      } catch {
        // keep unknown
      }
    }
    const envDatasetId = process.env.CE_QA_DATASET_ID?.trim() || '';
    const envDatasetHash = process.env.CE_QA_DATASET_HASH?.trim() || '';
    const useEnvDatasetPair = envDatasetId.length > 0 && envDatasetHash.length > 0;
    const useArtifactDatasetPair = artifactDatasetId.length > 0 && artifactDatasetHash.length > 0;
    const reproducibilityDatasetId = useEnvDatasetPair
      ? envDatasetId
      : useArtifactDatasetPair
        ? artifactDatasetId
        : defaultDatasetId;
    const reproducibilityDatasetHash = useEnvDatasetPair
      ? envDatasetHash
      : useArtifactDatasetPair
        ? artifactDatasetHash
        : 'unknown';
    ...
    const output: OutputArtifact = {
      ...
      inputs: {
        fixture_pack: fixtureResolved,
        holdout_artifact: holdoutCheckPath,
      ...
      reproducibility_lock: {
        ...
        config_snapshot: {
          fixture_pack: fixtureResolved,
          telemetry_artifact: path.resolve('artifacts', 'bench', 'retrieval-quality-telemetry.json'),
          holdout_artifact: holdoutCheckPath,
